...
async function getcursesources(req, res) {
    ...
    if (month <= 6) {
        const Curses = await sourcecurse_1.default.find({ ciclo: { $in: ['1', '3', '5', '7', '9'] }, mencion: { $in: ['ED', 'G', 'P', 'E'] } });
        return res.json(Curses);
    }
    else {
        const Curses = await sourcecurse_1.default.find({ ciclo: { $in: ['2', '4', '6', '8', '10'] }, mencion: { $in: ['ED', 'G', 'P', 'E'] } });
        return res.json(Curses);
    }
    //return res.json("Curses");
}
