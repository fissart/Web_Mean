...
async function getsController(req, res) {
    ...
    console.log(type);
    const data = await _7_Collection_1.default.find({ type: type });
...
async function updateController(req, res) {
    ...
    if (req.file) {
        ...
        const update = await _7_Collection_1.default.findByIdAndUpdate(id, { title, description, file: req.file.path });
    }
    else {
        const update = await _7_Collection_1.default.findByIdAndUpdate(id, { title, description });
    }
    return res.json({
